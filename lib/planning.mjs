...
// "How far are these six slider positions from doing nothing" — mirrors
// the mockup's own planningFactors(): adoption, usage, tokens and a 70/30
// blend of company/provider token price each scale spend proportionally to
// their ratio vs baseline; model mix applies an assumed 0.8% spend change
// per percentage-point shift toward higher-cost models. `defaults` is the
// "no adjustment" center point for every slider — always seeded from real
// current-period data by the caller (see buildPlanningDefaults() in
// index.html), never hardcoded here. `adoptionFactor` is exposed on its own
// (not just folded into usageFactor) because the chart's Adoption line
// scales its own baseline projection by this ratio directly, the same way
// Spend/Usage scale by spendFactor/usageFactor.
export function computeScenarioFactors(values, defaults) {
  ...
  return {
    spendFactor: adoption * usage * tokens * price * modelMix,
    usageFactor: adoption * usage,
